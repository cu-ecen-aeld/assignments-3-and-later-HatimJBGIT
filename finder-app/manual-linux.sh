...
if [ ! -e ${OUTDIR}/linux-stable/arch/${ARCH}/boot/Image ]; then
    ...
    echo "[DEBUG] 1 configuring mproper"
    make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} mrproper
    echo "[DEBUG] 1 configuring defconfig"
    make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} defconfig
    echo "[DEBUG] 1 configuring all"
    make -j 4 ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} all
# make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} modules
    echo "[DEBUG] 1 configuring dtbs"
    make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} dtbs
    echo "[DEBUG] 2 configuring dtbs"
fi
...
LIBC_PATH=~/cross_compiler/arm-cross-compiler/gcc-arm-10.3-2021.07-x86_64-aarch64-none-linux-gnu/aarch64-none-linux-gnu/libc
